...
- `usethis-doc-edit`: Guidelines for editing documentation — including scope matching, section placement, and content organisation
...
- `usethis-qa-config-files-sync`: Check whether docs/config-files.txt (the machine-readable export) is in-sync with the per-tool tables in docs/about/config-files.md
- `usethis-qa-doc-freshness`: Review documentation for outdated or inaccurate content by cross-referencing the current source code and project capabilities
...
- ALWAYS invoke the `usethis-python-test-full-coverage` skill — it is mandatory for any task that involves writing or modifying code, without exception. Use this skill to write tests and verify full coverage before finishing. Aim for 100% coverage on new or changed code.
